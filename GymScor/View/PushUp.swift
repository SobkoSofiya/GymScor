        ...
        ZStack{
            Rectangle()
                ...
                .background(LinearGradient(gradient: Gradient(colors: [Color("l"), Color("t")]), startPoint: .leading
                                           , endPoint: .trailing))
            VStack{
                Text("Push ups").foregroundColor(.white).font(.custom("ND Astroneer", size: 24)).padding(.bottom)
...
struct PushUp_Previews: PreviewProvider {
    static var previews: some View {
        PushUp( transitionTren: .constant(1))
